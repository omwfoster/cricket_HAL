...
seesaw_NeoPixel::seesaw_NeoPixel(uint16_t n, uint8_t p, neoPixelType t,
    ...
    : Adafruit_seesaw(hi2c), begun(false), numLEDs(n), pin(p), brightness(0),
...
seesaw_NeoPixel::seesaw_NeoPixel(I2C_HandleTypeDef *hi2c)
    : Adafruit_seesaw(hi2c),
#ifdef NEO_KHZ400
      is800KHz(true),
#endif
...
void seesaw_NeoPixel::show(void) {
 ...
 // while (!canShow()) TODO: replace
    ;

  this->write(SEESAW_NEOPIXEL_BASE, SEESAW_NEOPIXEL_SHOW, NULL, 0);

//  endTime = micros(); TODO: replace // Save EOD time for latch on next call
}
